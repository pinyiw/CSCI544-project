...
acos_base_dir = (Path(__file__).parent.parent.parent / 'dataset/ACOS').resolve()
...
lines_to_read = -1
acos_reviews_train = load_acos_reviews(laptop_acos_train_file, lines_to_read)
acos_reviews_dev = load_acos_reviews(laptop_acos_dev_file, lines_to_read)


print(f"Loaded {len(acos_reviews_train)} training reviews")
print(f"Loaded {len(acos_reviews_dev)} dev reviews")
...
# print(generate_acos_pred_prompts(acos_reviews_train[:5], acos_reviews_dev[0].review))


# %%
# free trial 900k tokens
...
for i, acos_review in enumerate(acos_reviews_dev):
    if i < 121:
        continue
...
# print(pred_str)
# resp
# acos_pred.acos_preds[0].to_acos_str()
# acos_review
...
output_file = output_dir / f'acos_preds_laptop_dev_{openai_model}_train-prompt-{train_data_len}.jsonl'
with open(output_file, 'w') as f:
    for i, acos_pred in enumerate(acos_preds):
        # acos_strs: list[str] = [quad_pred.to_acos_str(acos_pred.review) for quad_pred in acos_pred.acos_preds]
        # f.write('\t'.join([acos_pred.review, *acos_strs]) + '\n')
        f.write(json.dumps(asdict(acos_pred)) + '\n')


#%%
# prompt = generate_acos_pred_prompts(acos_reviews_train[:train_data_len], acos_preds[4].review)
prompt = generate_acos_pred_prompts(acos_reviews_train[:10], acos_preds[11].review)
print(prompt)
